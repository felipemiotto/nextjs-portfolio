...
const Header = ({ navbarOpen, setNavbarOpen, isDarkMode }: HeaderProps) => {
  ...
  return (
    <div className='flex flex-row justify-between px-[8%] py-5 sm:py-8 lg:py-10 text-center'>
      ...
      theme{theme}
          ...
          <button
          className={`flex top-0 right-0 z-20 relative w-10 h-10 text-${theme === 'light' ? 'black' : 'white'} focus:outline-none`}
          onClick={() => setNavbarOpen(!navbarOpen)}
          >
            <div className="absolute w-5 transform -translate-x-1/2 -translate-y-1/2 left-1/2 top-1/2">
                <span
                  className={`absolute h-0.5 w-5 bg-${theme === 'light' ? 'black' : 'white'} transform transition duration-300 ease-in-out ${
                      navbarOpen ? "rotate-45 delay-200" : "-translate-y-1.5"
                  }`}
                ></span>
                <span
                  className={`absolute h-0.5 bg-${theme === 'light' ? 'black' : 'white'} transform transition-all duration-200 ease-in-out ${
